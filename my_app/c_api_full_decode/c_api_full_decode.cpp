...
int main() {
    ...
    // Setup decode printer
    ret = ocsd_dt_set_gen_elem_printer(dcdtree_handle);
    if (ret != OCSD_OK)
        return ret;
    
    // Add program image
    vector<uint8_t> program_image_buffer = readFileIntoBuffer("../simple_juno_trace/juno_snapshot/mem_Cortex-A53_0_0_EXEC.bin");
    ret = ocsd_dt_add_buffer_mem_acc(dcdtree_handle, 0x80000000, OCSD_MEM_SPACE_ANY, program_image_buffer.data(), program_image_buffer.size());
    if (ret != OCSD_OK)
        return ret;
